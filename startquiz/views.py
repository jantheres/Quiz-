...
class StartQuick(View):
    def  get(self, request, cat_id = None, index = None):
        ...
        qstn_links      = []
        context         = {
            'user_type':User.objects.get(id=user_id).user_type
        }

        context['username'] = User.objects.filter(id = user_id).first().username

        if cat_id is not None:
            all_questions = list(Question.objects.filter(category_id = cat_id, approved_status=1).exclude(added_by=user_id).order_by('id').values())
